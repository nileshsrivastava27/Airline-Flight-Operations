"""
Notebook-friendly Gold transformation helper for Databricks.

This module transforms Silver Delta tables into Gold reporting tables for the
airline flight operations Lakehouse project.
"""
...
class GoldTransformationJob:
    ...
    def run_all(self) -> list[dict[str, Any]]:
        results = [
            self._write_dataframe(
                ...
                GoldWriteSpec("airline_ops.gold.on_time_performance_daily"),
            ),
            self._write_dataframe(
                self.transform_route_delay_summary(),
                GoldWriteSpec("airline_ops.gold.route_delay_summary"),
            ),
            self._write_dataframe(
                self.transform_airport_delay_summary(),
                GoldWriteSpec("airline_ops.gold.airport_delay_summary"),
            ),
            self._write_dataframe(
                self.transform_cancellation_summary(),
                GoldWriteSpec("airline_ops.gold.cancellation_summary"),
            ),
            self._write_dataframe(
                self.transform_aircraft_performance_summary(),
                GoldWriteSpec("airline_ops.gold.aircraft_performance_summary"),
            ),
        ]
        return results
    ...
    def transform_on_time_performance_daily(self) -> DataFrame:
        ...
        flights_df = self.spark.table("airline_ops.silver.flight_operations_clean")
        return (
            ...
            .agg(
                ...
                F.sum(F.when(F.coalesce(F.col("departure_delay_minutes"), F.lit(0)) > 15, 1).otherwise(0)).alias(
                    "delayed_departure_flights"
                ),
                F.sum(F.when(F.coalesce(F.col("arrival_delay_minutes"), F.lit(0)) > 15, 1).otherwise(0)).alias(
                    "delayed_arrival_flights"
                ),
    ...
    def transform_airport_delay_summary(self) -> DataFrame:
        ...
        airport_perf_df = departures_df.unionByName(arrivals_df)

        return (
            airport_perf_df.groupBy("flight_date", "airport_code", "airport_role", "reporting_airline")
            .agg(
                F.count("*").alias("total_flights"),
                F.sum(F.when(~F.col("cancelled_flag"), 1).otherwise(0)).alias("completed_flights"),
                F.sum(F.when(F.col("cancelled_flag"), 1).otherwise(0)).alias("cancelled_flights"),
    ...
    def transform_aircraft_performance_summary(self) -> DataFrame:
        from pyspark.sql import functions as F

        flights_df = self.spark.table("airline_ops.silver.flight_operations_clean")
        aircraft_df = self.spark.table("airline_ops.silver.aircraft_reference_clean")

        aircraft_joined_df = flights_df.withColumn(
            "aircraft_code",
    ...
    def _write_dataframe(self, df: DataFrame, spec: GoldWriteSpec) -> dict[str, Any]:
